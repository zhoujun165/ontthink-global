...
class GlobalController extends AdminController{
	...
	private function _setFileInfo(){
		if(is_up_file()){
			...
			foreach ($upfile_infos as $v) {
				$field_name = $v['key'];
				if($up_config['driver'] == 'Aliyun'){
					$img_http_url = $up_config['oss_url'].$v['savepath'].$v['savename'];
				}else{
	...
	public function getSearchCondition($model = ''){
        ...
        if(strpos($model->getModelName(), 'View')>-1){
            $all_fields = $model->getViewFields();
        }else{
	...
	private function _checkModel($model){
		...
		if (is_string($model)) {
			if (ACTION_NAME == 'index') {
				if (class_exists('Admin\Model\\'.CONTROLLER_NAME . 'ViewModel')) {
    ...
    public function KindEditorUploadImgToAli(){
        $url = \Common\Utils\UploadUtil::uploadImg2Ali();
        if($url){
            echo json_encode(array('error' => 0, 'url' => $url));
        }else{
            echo json_encode(array('error' => 0, 'message' => "上传出错"));
        }
    }
}